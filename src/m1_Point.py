...
Authors: David Mutchler, Vibha Alangar, Dave Fisher, Amanda Stouder,
         their colleagues and LIAM GROOM.
...
class Point(object):
    ...
    def move_to(self,x,y):
        ...
        self.travel=self.travel+self.get_distance_from(b4move)
    def move_by(self,dx,dy):
        b5move=self.clone()
        self.x=self.x+dx
        self.y=self.y+dy
        self.moves = self.moves + 1
        self.travel=self.travel+self.get_distance_from(b5move)
    def get_number_of_moves_made(self):
        return self.moves
    def get_distance_from_start(self):
        dx=self.initialx-self.x
        dy=self.initialy-self.y
        dr = (((dx) ** 2) + ((dy) ** 2)) ** (1 / 2)
        return dr
    ...
    def halfway_to(self,p2):
        new=Point(((self.x+p2.x)/2),((self.y+p2.y)/2))
        return new
